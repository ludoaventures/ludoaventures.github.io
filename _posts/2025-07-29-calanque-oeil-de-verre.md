---
title: Calanques de l'oeil de verre
description: Randonnée jusqu'à la Calanque de l'oeil de verre par la corniche du CAF
date: 2025-07-29 11:33:00 +0800
categories: [Randonnée]
tags: [Calanques]
pin: false
math: true
mermaid: true

---

## 📍 Lieu & Informations Générales

- **Lieu de départ :** Parking de Luminy
- **Coordonnées GPS :** 43.229754, 5.437045
- **Date :** 29/07/2025
- **Durée estimée :** 4h
- **Distance :** 11 km
- **Dénivelé :** +520 m / -520 m
- **Niveau :** Pas de difficulté particulière mais être à l'aise en montagne, quelques passages exposés
- **Type de parcours :** Boucle
- **Accessible en transports en commun :** Oui
- **Conditions météo :** Grand soleil avec du mistral


🚗 **[🧭 Se rendre au départ](https://www.google.com/maps/dir/?api=1&destination=43.229797,5.437043)**

---

## 🗺️ Carte interactive Komoot

<iframe src="https://www.komoot.com/fr-fr/tour/2448728821/embed?share_token=aGw1uSuuWEtrYRAF3MB9dy0hnlC1Htoyolobz5m4tg84QTeW93&profile=1" width="100%" height="700" frameborder="0" scrolling="no"></iframe>

---

> **Attention :** Renseignez vous sur l'ouverture des massifs en fonction des prévisions de risque incendies sur: [Carte du risque incendie](https://www.risque-prevention-incendie.fr/13)
{: .prompt-danger }

## 📌 Étapes du Parcours

#### 1. Départ – Parking de Luminy
Se garer entre l'Ecole des beaux arts et Kedge. Suivre les panneaux jaunes vers le col de Sugiton

#### 2. Col de Sugiton
Une fois arrivé au col de sugiton, prendre le GR (marque rouges et blanches) sur la gauche en suivant le sentier

#### 3. Socle de la Candelle
En suivant le GR on se dirige vers le socle de la Candelle sur lequel repose l'arête. Avant d'arriver au socle, deux option:
  - Voie normale (sur le GPX): un premier chemin monte au dessus de la falaise à gauche et mène au col de la Candelle
  - Variante (pas suivie sur le GPX): un sentier plus loin monte raide mais directement jusqu'à la fissure du Candelon

#### 4. Col de la Candelle
Après la montée suivre le chemin jusqu'au col de la Candelle. Un petit peu avant un sentier descend dans la pente et rentre dans le goulet au pied de l'arête. Les traces de pas sont évidentes et à un moment on retrouve un balisage noir. Ce dernier mène à un sentier exposé qui relie le haut du goulet par un pas exposé et glissant mais bien équipé avec des chaines

#### 4 (variante). Fissure du Candelon
En arrivant du bas remonter la fissure qui est encaissée et très raide.

#### 5. Candelon
Après le pas équipé de chaînes, descendre dans la fissure (ne pas essayer de remonter) et prendre un sentier peu évident mais balisé qui part sur la gauche pour contourner le Candellon par le bas.

#### 6. Corniche du CAF falaise Sud de la Candelle
Arrivé sur ce "plateau", vous retrouverez le départ de "L'arête de Marseille" grande voie mythique qui remonte l'arête par le pas de Rebuffat. Si vous ne grimpez pas, suivez les balisages "++" noirs qui serpentent sur le plateau.

#### 7. Succession de pas
Depuis le haut du socle de la Candelle, descendre le sentier "++" qui enchaîne une succession de pas exposés mais pas difficile jusqu'à rejoindre le haut de la Calanque de l'oeil de verre.

#### 8. Calanque de l'oeil de verre
La descente jusqu'à la Calanque se fait par le Pas de l'oeil de verre, patoné et exposé mais très bien équipé. Attention à ne pas poursuivre trop loin sur le sentier des Corniches Paretti (très joli mais beaucoup plus long jusqu'au Cirque du Devenson).

#### 9. Retour
Le retour est évident en suivant le sentier qui longe la mer et remonte jusqu'au Col de Sugiton. Très beau Panorama sur la Calanque de Sugiton et le Torpilleur.
  
---


## 📝 Remarques personnelles

Parcours de replis car trop de mistral pour faire l'arête de Marseille (plan initial). L'accès à la calanque de Sugiton était restreint mais en expliquant aux gardes que nous allions à côté pas de soucis pour passer. 
Le couloir du Candelon est bien équipé avec des chaînes mais très glissant (praticable en temps de pluie ?).
Vérifier que le Parc des Calanques est ouvert (voir dans les liens utiles) en fonction du risque incendie. Le département annonce pour chaque massif la veille en fonction des prévisions.

Remarque: L'accès à la calanque de sugiton est restreint l'été et il est nécessaire de réserver entre J-3 et J-1. Pour cet itinéraire on ne passe pas par la calanques donc les bénévoles nous ont laissé passer sans problème.

---

## 🧭 Conseils & Équipement

- Chaussures recommandées : [Chaussures basses / montantes / crampons]
- Eau : 1,5L par personne, tout juste
- Protection : Crème solaire, casquette et lunettes car grand soleil
- Autres : [Cartes, applis utilisées, durée des pauses...]

---

## 🔗 Ressources utiles

- [Ouverture des massifs selon le risque incendie](https://www.risque-prevention-incendie.fr/bouches-du-rhone)
- [Lien de réservation Calanques de Sugiton](https://www.calanques-parcnational.fr/fr/des-decouvertes/preparer-sa-visite/reservation-sugiton?gad_source=1&gad_campaignid=17669964230&gclid=EAIaIQobChMIv9_Kl__kjgMVX8p5BB0m6zJkEAAYASAAEgKoP_D_BwE)
- [Topo "arête de Marseille"](https://www.camptocamp.org/routes/55467/fr/la-grande-candelle-arete-de-marseille)
- [Topo "Corniches Paretti"](https://www.camptocamp.org/routes/417745/fr/calanques-devenson-paretti-corniche-paretti)
